...
if ! { [ "$TERM" = "screen" ] && [ -n "$TMUX" ]; } then
  echo "You are not running the installer with tmux"
  echo "We STRONGLY reccomend running the install in tmux"
  echo "Press etner to continue or CTRL + C to exit"
fi
...
if [ "$LOCATION" == "/home/ratus5/Downloads/temp-report" ]
then
  echo "Correct install location found"
else
  echo "Please move the folder to /home/pi, so that the path to this file is /home/pi/temp-report/install.sh and run this script there"
  exit
fi
...
echo "Use 'python3.6 temp.py -s' to set a sender email address"
echo "Use 'python3.6 temp.py -n' to set a sender name"
echo "Use 'python3.6 temp.py -p' to update the password for the sender address"
echo "Use 'python3.6 temp.py -a' to add or edit addresses on the mailing list"
